const fs = require('fs');
let code = fs.readFileSync('c:/KiloApps/KiloApps/KBBS/main.c', 'utf8');
...
code = code.replace("void ProcessTransferByte(unsigned char ch) {", zmodem_funcs);

// 3. Add ZMODEM handling to ProcessTransferByte
const zmodem_proc = `    if (transferActive == 3 || transferActive == 4) {
...
code = code.replace("    if (transferActive == 1) { /* Xmodem DL */", zmodem_proc + "    if (transferActive == 1) { /* Xmodem DL */");

// 4. Modify ProcessTelnetByte to auto-detect ZRQINIT
const telnet_detect = `        case TEL_STATE_NORMAL:
            if (ch == TELNET_IAC) {
...
const ui_btns = `            hBtnXmDl = CreateWindowA("BUTTON", "DL (XM)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
...
const regex_btns = /hBtnXmDl = CreateWindowA[\s\S]*?\(HMENU\)110, 0, 0\);/;
code = code.replace(regex_btns, ui_btns);

code = code.replace("SendMessageA(hBtnXmUl, WM_SETFONT, (WPARAM)hFont, TRUE);", 
...
// 6. Button events for 105 and 106
const regex_cmd = /ofn\.Flags = OFN_PATHMUSTEXIST \| OFN_FILEMUSTEXIST;[\s\S]*?InvalidateRect\(hwnd, NULL, FALSE\);\s*\}\s*\}\s*\}\s*\}\s*\} else if \(LOWORD\(wParam\) == 110\) \{/;
code = code.replace(regex_cmd, 
...
const overlay = `DrawTextA(winDC, transferActive == 1 ? "XMODEM DOWNLOAD" : "XMODEM UPLOAD", -1, &rTitle, DT_CENTER | DT_TOP);`;
...
fs.writeFileSync('c:/KiloApps/KiloApps/KBBS/main.c', code);
