...
void TriggerEmp() {
    ...
    for (int i = 0; i < 60; i++) {
        ...
        float ang = (i / 60.0f) * 3.14159f * 2.0f;
        p->x = ship.x;
        p->y = ship.y;
        p->vx = cos(ang) * 8.0f;
        p->vy = sin(ang) * 8.0f;
...
void TriggerHyperdrive() {
    ...
    for (int i = 0; i < 25; i++) {
        if (num_particles >= 600) break;
        Particle* p = &particles[num_particles++];
        p->x = ship.x; p->y = ship.y;
        p->vx = ((rand() % 100) - 50) / 10.0f;
        p->vy = ((rand() % 100) - 50) / 10.0f;
        p->life = 25; p->max_life = 25;
        p->color = RGB(56, 189, 248); p->active = true;
    }

    // Find safe coordinate
    float nx, ny;
    bool safe = false;
...
void KillShip() {
    ...
    for (int p = 0; p < 50; p++) {
        ...
        pt->vx = ((rand() % 100) - 50) / 8.0f;
        pt->vy = ((rand() % 100) - 50) / 8.0f;
        pt->life = 40 + (rand() % 30);
        pt->max_life = pt->life;
        pt->color = RGB(239, 68, 68);
        pt->active = true;
    }
}
            ...
            for (int m = 0; m < num_mines; m++) {
                ...
                if (dist < mines[m].radius) {
                    ...
                    for (int p = 0; p < 20; p++) {
                        ...
                        pt->life = 30 + (rand() % 20); pt->max_life = pt->life; pt->color = RGB(168, 85, 247); pt->active = true;
                    }
                    score += 150;
                    stats.shots_hit++;
                    current_shots_hit++;
            ...
            for (int k = 0; k < num_ufos; k++) {
                ...
                if (dist < ufos[k].radius) {
                    ...
                    for (int p = 0; p < 10; p++) {
                        ...
                        pt->life = 20; pt->max_life = 20; pt->color = RGB(239, 68, 68); pt->active = true;
                    }
            ...
            for (int j = 0; j < num_asteroids; j++) {
                ...
                if (dist < asteroids[j].radius) {
                    ...
                    for (int p = 0; p < 15; p++) {
                        ...
                        pt->vx = ((rand() % 100) - 50) / 12.0f; pt->vy = ((rand() % 100) - 50) / 12.0f;
                        pt->life = 25; pt->max_life = 25;
                        pt->color = asteroids[j].is_armored ? RGB(249, 115, 22) : RGB(148, 163, 184);
                        pt->active = true;
                    }
...
void Update() {
    ...
    if (keys[VK_UP]) {
        ...
        if (num_particles < 600 && ship.active) {
            ...
            p->color = RGB(249, 115, 22); p->active = true;
        }
    }
    ...
    if (keys[VK_SPACE]) {
        ...
        if (now - last_shoot_time > cooldown && num_bullets < 100) {
            ...
            if (spread_timer > 0) {
                ...
                b->x = ship.x + cos(ship.angle) * 15.0f;
                b->y = ship.y + sin(ship.angle) * 15.0f;
                b->vx = cos(ship.angle) * 8.0f;
                b->vy = sin(ship.angle) * 8.0f;
                b->life = 60;
                b->active = true;
                b->is_enemy = false;
                b->is_laser = (laser_timer > 0);
    ...
    for (int i = 0; i < num_ufos; i++) {
        ...
        if ((ufos[i].vx > 0 && ufos[i].x > WIDTH + 50) || (ufos[i].vx < 0 && ufos[i].x < -50)) {
            ...
            else ufos[i].vx = -ufos[i].vx; // Bounce Boss
        }
...
void Draw(HDC hdc) {
    ...
    HPEN armAstPen = CreatePen(PS_SOLID, 2, RGB(249, 115, 22));
    HPEN bulletPen = CreatePen(PS_SOLID, 2, RGB(253, 224, 71));
    HPEN laserPen = CreatePen(PS_SOLID, 3, RGB(239, 68, 68));
    ...
    for (int i = 0; i < num_asteroids; i++) {
        if (!asteroids[i].active) continue;
        SelectObject(hdc, asteroids[i].is_armored ? armAstPen : astPen);
        POINT pts[10];
        for (int j = 0; j < 10; j++) {
    ...
    for (int i = 0; i < num_ufos; i++) {
        if (!ufos[i].active) continue;
        SelectObject(hdc, (ufos[i].type == 1) ? bossPen : ufoPen);
        float r = ufos[i].radius;
        MoveToEx(hdc, (int)(ufos[i].x - r), (int)ufos[i].y, NULL);
        LineTo(hdc, (int)(ufos[i].x + r), (int)ufos[i].y);
        LineTo(hdc, (int)(ufos[i].x + r * 0.5f), (int)(ufos[i].y - r * 0.5f));
        LineTo(hdc, (int)(ufos[i].x - r * 0.5f), (int)(ufos[i].y - r * 0.5f));
        LineTo(hdc, (int)(ufos[i].x - r), (int)ufos[i].y);
        LineTo(hdc, (int)(ufos[i].x - r * 0.5f), (int)(ufos[i].y + r * 0.5f));
        LineTo(hdc, (int)(ufos[i].x + r * 0.5f), (int)(ufos[i].y + r * 0.5f));
        ...
        if (ufos[i].type == 1) { // Boss HP Bar
            HBRUSH hpBg = CreateSolidBrush(RGB(63, 63, 70));
            HBRUSH hpFg = CreateSolidBrush(RGB(34, 197, 94));
            RECT barBg = {(LONG)(ufos[i].x - 30), (LONG)(ufos[i].y - 38), (LONG)(ufos[i].x + 30), (LONG)(ufos[i].y - 32)};
            FillRect(hdc, &barBg, hpBg);
            float pct = (float)ufos[i].hp / ufos[i].max_hp;
            RECT barFg = {(LONG)(ufos[i].x - 30), (LONG)(ufos[i].y - 38), (LONG)(ufos[i].x - 30 + 60 * pct), (LONG)(ufos[i].y - 32)};
    ...
    for (int i = 0; i < num_bullets; i++) {
        ...
        if (bullets[i].is_enemy) {
            ...
            MoveToEx(hdc, (int)(bullets[i].x - bullets[i].vx), (int)(bullets[i].y - bullets[i].vy), NULL);
            LineTo(hdc, (int)(bullets[i].x + bullets[i].vx), (int)(bullets[i].y + bullets[i].vy));
        } else {
    ...
    DeleteObject(astPen);
    DeleteObject(armAstPen);
    DeleteObject(ufoPen);
    DeleteObject(bossPen);
