const fs = require('fs');
let code = fs.readFileSync('KBBS/main.c', 'utf8');

const defs = `#define IDD_DIALDIR 1000
...
code = code.replace("void ClearScreen(void) {", defs + "\nvoid ClearScreen(void) {");

const btn_str = `hCombo = CreateWindowA("BUTTON", "Directory", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                dpiScale(428), dpiScale(4), dpiScale(100), dpiScale(22), hwnd, (HMENU)101, 0, 0);`;

code = code.replace(/hCombo\s*=\s*CreateWindowA\("COMBOBOX"[^;]+;/, btn_str);

code = code.replace(/\/\*\s*Populate BBS list\s*\*\/[\s\S]*?\}/, '');
...
code = code.replace(/else if \(LOWORD\(wParam\) == 101 && HIWORD\(wParam\) == CBN_SELCHANGE\) \{[\s\S]*?break;\s*\}/, dir_btn_logic + "            break;\n        }");

fs.writeFileSync('KBBS/main.c', code, 'utf8');
console.log("done");
