...
"""
c = c.replace("#pragma function(memset)", headers + "\n#pragma function(memset)")
...
gameplay_keys = """
                ...
                void RecordInput(char k) { if(current_replay.count < 5000) { current_replay.events[current_replay.count].tick = replay_tick; current_replay.events[current_replay.count].key = k; current_replay.count++; } }
                ...
                if (wParam == keys.nuke) { UseRowNuke(); RecordInput('B'); InvalidateRect(hwnd, NULL, FALSE); break; }
                if (wParam == keys.swap) { UsePieceSwap(); RecordInput('S'); InvalidateRect(hwnd, NULL, FALSE); break; }
                if (wParam == keys.freeze) { UseGravityFreeze(); RecordInput('F'); InvalidateRect(hwnd, NULL, FALSE); break; }

                if (is_paused) break;

                if (wParam == keys.left && !check_collision(current_piece, current_rot, current_x - 1, current_y)) { current_x--; RecordInput('L'); }
                if (wParam == keys.right && !check_collision(current_piece, current_rot, current_x + 1, current_y)) { current_x++; RecordInput('R'); }
                if (wParam == keys.down && !check_collision(current_piece, current_rot, current_x, current_y + 1)) { current_y++; RecordInput('D'); }
                ...
                if (wParam == keys.hold || wParam == VK_SHIFT) {
                    if (!hold_used) {
                        ...
                        hold_used = 1; Beep(700, 30); RecordInput('C');
                    }
                }
                if (wParam == keys.drop) {
