...
- **KContacts**: In `kcontacts.html`, implemented a robust RFC-compliant CSV tokenizer (`parseCsvLine`) supporting quotes with embedded commas/newlines, added RFC 2426 continuation line unfolding and fallback `N:` surname/given name parsing for vCards, sanitized phone and email URI scheme targets in Markdown directory export to prevent space-induced link breakage, fixed `btnMerge` duplicate removal to preserve surviving contact selection, added `tabindex="0"`, `role="tab"`, and `onkeydown` keyboard navigation to category and tag pills, and added dialog ARIA attributes (`role="dialog"`, `aria-modal="true"`, `aria-labelledby`) across Help, Export, and Delete modals. In `main.c`, resolved zero font-size bug in `WM_CREATE` by initializing `g_dpi` before computing `fontHeight`, implemented CSV import (`parse_csv_field`) supporting quoted fields and custom headers, added multiline notes preservation in `Save Details` using `extract_multiline_field`, added CRLF escaping for notes in vCard exports, added `WS_TABSTOP` styles and integrated `IsDialogMessage` message loop routing for Win32 keyboard navigation, and preserved adjacent contact selection on contact deletion. Verified native compilation readiness and Vite web build cleanly.
