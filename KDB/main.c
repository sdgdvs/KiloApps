...
void SortRecords(int col, int asc) {
    if (data_count <= 1 || col < 0 || col > 3) return;
    for (int i = 0; i < data_count - 1; i++) {
        for (int j = 0; j < data_count - i - 1; j++) {
            int cmp = 0;
            if (col == 0) {
                int numA = atoi(data[j].id);
                int numB = atoi(data[j + 1].id);
                if (numA > 0 && numB > 0 && numA != numB) {
                    cmp = (numA > numB) ? 1 : -1;
                } else {
                    cmp = lstrcmpiA(data[j].id, data[j + 1].id);
                }
            } else if (col == 1) {
                cmp = lstrcmpiA(data[j].name, data[j + 1].name);
            } else if (col == 2) {
                cmp = lstrcmpiA(data[j].dept, data[j + 1].dept);
            } else if (col == 3) {
                cmp = lstrcmpiA(data[j].role, data[j + 1].role);
            }
            if (!asc) cmp = -cmp;
            if (cmp > 0) {
                Record temp = data[j];
...
void AutoScaleListViewColumns(int totalWidth) {
    ...
    int colName = (remaining * 38) / 100;
    int colDept = (remaining * 31) / 100;
    int colRole = (remaining * 31) / 100;
...
void WriteCsvField(HANDLE hFile, const char* str) {
    DWORD w;
    WriteFile(hFile, "\"", 1, &w, NULL);
    if (str) {
        const char* p = str;
        while (*p) {
            if (*p == '"') {
                WriteFile(hFile, "\"\"", 2, &w, NULL);
            } else if (*p != '\r' && *p != '\n') {
                WriteFile(hFile, p, 1, &w, NULL);
            }
            p++;
        }
    }
...
void StripFieldQuotes(char* str) {
    if (!str) return;
...
void ExportCSV(HWND hwnd) {
    ...
    if (PromptFile(hwnd, path, 1, "CSV Files\0*.csv\0All Files\0*.*\0", "csv")) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            for (int i = 0; i < data_count; i++) {
                WriteCsvField(hFile, data[i].id);
                WriteFile(hFile, ",", 1, &w, NULL);
                WriteCsvField(hFile, data[i].name);
                WriteFile(hFile, ",", 1, &w, NULL);
                WriteCsvField(hFile, data[i].dept);
                WriteFile(hFile, ",", 1, &w, NULL);
                WriteCsvField(hFile, data[i].role);
                WriteFile(hFile, "\r\n", 2, &w, NULL);
            }
            CloseHandle(hFile);
            MessageBoxA(hwnd, "Exported CSV successfully.", "Success", MB_OK);
        }
    }
}
...
void ImportCSV(HWND hwnd) {
    ...
    if (PromptFile(hwnd, path, 0, "CSV Files\0*.csv\0All Files\0*.*\0", "csv")) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
                    ...
                    while (*p && data_count < MAX_RECORDS) {
                        ...
                        if (line > 0 && lstrlenA(p) > 3) {
                            ...
                            if (id && name && dept && role) {
                                StripFieldQuotes(id);
                                StripFieldQuotes(name);
                                StripFieldQuotes(dept);
                                StripFieldQuotes(role);
                                char* ret = (char*)FindChar(role, '\r'); if (ret) *ret = 0;
                                
                                int dup = 0;
                                for (int i = 0; i < data_count; i++) {
                                    if (lstrcmpiA(data[i].id, id) == 0) { dup = 1; break; }
                                }
                                if (!dup && *id && *name) {
            ...
            CloseHandle(hFile);
            SaveDataToFile();
            char msg[128]; wsprintfA(msg, "Imported CSV. Total records: %d", data_count);
            MessageBoxA(hwnd, msg, "Success", MB_OK);
        }
    }
}

void WriteJsonEscaped(HANDLE hFile, const char* str) {
    DWORD w;
    WriteFile(hFile, "\"", 1, &w, NULL);
    if (str) {
        const char* p = str;
...
void ExportJSON(HWND hwnd) {
    ...
    if (PromptFile(hwnd, path, 1, "JSON Files\0*.json\0All Files\0*.*\0", "json")) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            WriteFile(hFile, "[\r\n", 3, &w, NULL);
            for (int i = 0; i < data_count; i++) {
                WriteFile(hFile, "  {\"id\":", 8, &w, NULL);
                WriteJsonEscaped(hFile, data[i].id);
                WriteFile(hFile, ", \"name\":", 9, &w, NULL);
                WriteJsonEscaped(hFile, data[i].name);
                WriteFile(hFile, ", \"dept\":", 9, &w, NULL);
                WriteJsonEscaped(hFile, data[i].dept);
                WriteFile(hFile, ", \"role\":", 9, &w, NULL);
                WriteJsonEscaped(hFile, data[i].role);
                if (i == data_count - 1) {
                    WriteFile(hFile, "}\r\n", 3, &w, NULL);
                } else {
                    WriteFile(hFile, "},\r\n", 4, &w, NULL);
            ...
            WriteFile(hFile, "]\r\n", 3, &w, NULL);
            CloseHandle(hFile);
            MessageBoxA(hwnd, "Exported JSON successfully.", "Success", MB_OK);
        }
    }
}
...
void ImportJSON(HWND hwnd) {
    ...
    if (PromptFile(hwnd, path, 0, "JSON Files\0*.json\0All Files\0*.*\0", "json")) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            if (size > 0 && size < 5000000) {
                ...
                if (buf) {
                    ...
                    while (*p && data_count < MAX_RECORDS) {
                        ...
                        if (id[0] && name[0]) {
                            int dup = 0; for (int i = 0; i < data_count; i++) if (lstrcmpiA(data[i].id, id) == 0) { dup = 1; break; }
                            if (!dup) {
            ...
            CloseHandle(hFile);
            SaveDataToFile();
            char msg[128]; wsprintfA(msg, "Imported JSON. Total records: %d", data_count);
            MessageBoxA(hwnd, msg, "Success", MB_OK);
        }
    }
}

void ExportMarkdown(HWND hwnd) {
    char path[MAX_PATH];
    if (PromptFile(hwnd, path, 1, "Markdown Files\0*.md\0All Files\0*.*\0", "md")) {
        HANDLE hFile = CreateFileA(path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (hFile != INVALID_HANDLE_VALUE) {
            DWORD w;
            char depts[64][64];
            ...
            for (int i = 0; i < data_count; i++) {
                int pid = atoi(data[i].id);
                if (pid > maxId) maxId = pid;
                
                int found = 0;
                for (int d = 0; d < dept_total; d++) {
                    if (lstrcmpiA(depts[d], data[i].dept) == 0) {
                        dept_counts[d]++;
                        found = 1;
                        break;
                    }
                }
                if (!found && dept_total < 64) {
                    lstrcpynA(depts[dept_total], data[i].dept, 64);
                    dept_counts[dept_total] = 1;
                    dept_total++;
                }
            }

            char buf[512];
            const char* title = "# 📁 KDB Employee Directory & Analytics Report\r\n\r\n## 📊 Executive Summary\r\n";
            WriteFile(hFile, title, lstrlenA(title), &w, NULL);

            wsprintfA(buf, "- **Total Active Records**: %d\r\n- **Departments**: %d\r\n- **Next Suggested ID**: %d\r\n\r\n",
                data_count, dept_total, maxId > 0 ? (maxId + 1) : (data_count + 101));
            WriteFile(hFile, buf, lstrlenA(buf), &w, NULL);

            const char* deptHdr = "## 🏢 Department Breakdown\r\n\r\n| Department | Headcount |\r\n| :--- | :--- |\r\n";
            WriteFile(hFile, deptHdr, lstrlenA(deptHdr), &w, NULL);

            for (int d = 0; d < dept_total; d++) {
                wsprintfA(buf, "| %s | %d |\r\n", depts[d], dept_counts[d]);
                WriteFile(hFile, buf, lstrlenA(buf), &w, NULL);
            }

            const char* rosterHdr = "\r\n## 👥 Employee Roster\r\n\r\n| ID | Full Name | Department | Role |\r\n| :--- | :--- | :--- | :--- |\r\n";
            WriteFile(hFile, rosterHdr, lstrlenA(rosterHdr), &w, NULL);
            ...
            const char* footer = "\r\n---\r\n*KDB - KiloApps High Performance Lightweight Suite*\r\n";
            WriteFile(hFile, footer, lstrlenA(footer), &w, NULL);

            CloseHandle(hFile);
            MessageBoxA(hwnd, "Exported Markdown report successfully.", "Success", MB_OK);
        }
    }
}
...
void ShowStats(HWND hwnd) {
    ...
    char msg[2048];
    char temp[128];
    wsprintfA(msg, "📊 KDB Database Analytics & Metrics\r\n\r\nTotal Employees: %d\r\nDepartments: %d\r\nNext Suggested ID: %d\r\n\r\nDepartment Headcount Breakdown:\r\n",
        data_count, dept_total, maxId > 0 ? (maxId + 1) : (data_count + 101));

    for (int d = 0; d < dept_total; d++) {
        int pct = data_count > 0 ? ((dept_counts[d] * 100) / data_count) : 0;
        wsprintfA(temp, "  • %s: %d (%d%%)\r\n", depts[d], dept_counts[d], pct);
        if (lstrlenA(msg) + lstrlenA(temp) + 128 < (int)sizeof(msg)) {
            lstrcatA(msg, temp);
        }
    }

    if (lstrlenA(msg) + 100 < (int)sizeof(msg)) {
        lstrcatA(msg, "\r\nTip: Double-click an employee to edit, click headers to sort.");
    }
...
void InitListView(HWND hwnd) {
    ...
    hAddId = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | WS_TABSTOP, 0, 0, 0, 0, hwnd, (HMENU)IDC_ADD_ID, GetModuleHandle(NULL), NULL);
    ...
    SendMessageA(hSearch, EM_SETCUEBANNER, FALSE, (LPARAM)"Search / Query (F1 for Help)...");
    ...
    switch (msg) {
        ...
        case WM_NOTIFY: {
            ...
            if (pnmh->hwndFrom == hListView) {
                if (pnmh->code == LVN_COLUMNCLICK) {
                    ...
                    SortRecords(g_sortCol, g_sortAsc);
                    char buf[128] = {0};
                    ...
                    if (sel != -1) {
                        ...
                        if (SendMessage(hListView, LVM_GETITEMA, 0, (LPARAM)&lvi)) {
                            ...
                            if (idx >= 0 && idx < data_count) {
                                SetWindowTextA(hAddId, data[idx].id);
                                SetWindowTextA(hAddName, data[idx].name);
                                SetWindowTextA(hAddDept, data[idx].dept);
                                SetWindowTextA(hAddRole, data[idx].role);
                                SetFocus(hAddName);
                            }
                        }
                    }
        ...
        case WM_COMMAND: {
            if (LOWORD(wParam) == IDC_SEARCH && HIWORD(wParam) == EN_CHANGE) {
                ...
                if (g_sortCol >= 0) SortRecords(g_sortCol, g_sortAsc);
                SaveDataToFile();
                ...
                if (sel != -1) {
                    ...
                    if (SendMessage(hListView, LVM_GETITEMA, 0, (LPARAM)&lvi)) {
                        ...
                        if (idx >= 0 && idx < data_count) {
                            ...
                            int totalVisible = SendMessage(hListView, LVM_GETITEMCOUNT, 0, 0);
                            if (totalVisible > 0) {
                                int newSel = (sel < totalVisible) ? sel : (totalVisible - 1);
                                ListView_SetItemState(hListView, newSel, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
                                ListView_EnsureVisible(hListView, newSel, FALSE);
                            }
                        }
                    }
                } else {
