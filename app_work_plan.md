...
- KBBS (Added Door Games Mini-Suite [LORD/TradeWars text adventure], ANSI Color & Art Viewer Mode, Multi-Node Dialing Directory Manager, Session Log Capture & Export [ANSI/TXT], Custom BBS Script Macro Auto-Login, and BBS Message Base & FidoNet EchoMail Conference Reader Suite)
...
- KMandel (Added Multi-threading, Custom Palettes, 4K Export, Julia Sets, Fractal Formula Engine [Mandelbrot, Burning Ship, Tricorn, Celtic, Buffalo], and Curated Landmark Presets Explorer)
- KPing (Added Packet Logging, Jitter Analytics, Payload Config, Log Export, Path MTU Discovery Sweep, Don't-Fragment [DF] Bit Diagnostics, and Quick Target Presets)
- KConnect4 (Added Save/Load States, Match Replay Viewer, Stats Tracker, JSON Export, Live Positional Evaluation Advantage Bar, Threat Radar, Connect-4 Move Notation [C4N & FEN] Suite with Position Setup & Match File Export, and Replay Auto-Play)
