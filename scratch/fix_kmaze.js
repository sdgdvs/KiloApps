const fs = require('fs');
let content = fs.readFileSync('d:\\KiloApps\\KMaze\\main.c', 'utf8');
...
// Replace NextLevel
let next_level_orig = `void NextLevel() {
    keysHeld = 0;
    currentLevel++;
...
let next_level_new = `void NextLevel() {
    keysHeld = 0;
    currentLevel++;
    if (currentLevel > 4) {
        gameState = 2;
...
let timer_start = `        case WM_TIMER: {
            float moveSpeed = 0.1f;`;
let timer_new = `        case WM_TIMER: {
...
let create_orig = `        case WM_CREATE:
            SetTimer(hwnd, 1, 30, NULL);
            break;`;
let create_new = `        case WM_CREATE:
            InitGame();
            SetTimer(hwnd, 1, 30, NULL);
            break;`;
content = content.replace(create_orig, create_new);

fs.writeFileSync('d:\\KiloApps\\KMaze\\main.c', content);
console.log("Updated main.c");
